...
namespace Kiri\AmqpLib\Channel;

use Closure;
use Kiri\AmqpLib\Connection\AbstractConnection;
use Kiri\AmqpLib\Exception\AMQPBasicCancelException;
use Kiri\AmqpLib\Exception\AMQPChannelClosedException;
use Kiri\AmqpLib\Exception\AMQPConnectionBlockedException;
use Kiri\AmqpLib\Exception\AMQPConnectionClosedException;
use Kiri\AmqpLib\Exception\AMQPNoDataException;
use Kiri\AmqpLib\Exception\AMQPProtocolChannelException;
use Kiri\AmqpLib\Exception\AMQPRuntimeException;
use Kiri\AmqpLib\Exception\AMQPTimeoutException;
use Kiri\AmqpLib\Helper\Assert;
use Kiri\AmqpLib\Message\AMQPMessage;
use Kiri\AmqpLib\Wire;
use Kiri\AmqpLib\Wire\AMQPReader;
use Kiri\AmqpLib\Wire\AMQPTable;
use Kiri\AmqpLib\Wire\AMQPWriter;
...
class AMQPChannel extends AbstractChannel
{
     ...
     * @param int|null $ticket
     * @return mixed
     * @throws AMQPTimeoutException if the specified operation timeout was exceeded
     */
    public function exchange_unbind(
        ...
        string          $routing_key = '',
        bool            $nowait = false,
        AMQPTable|array $arguments = array(),
        int             $ticket = null
    ): mixed
    {
        $ticket = $this->getTicket($ticket);
    ...
    public function queue_bind(
        ...
        string          $routing_key = '',
        bool            $nowait = false,
        array|AMQPTable $arguments = array(),
        ?int            $ticket = null
    ): mixed
    {
        $ticket = $this->getTicket($ticket);
